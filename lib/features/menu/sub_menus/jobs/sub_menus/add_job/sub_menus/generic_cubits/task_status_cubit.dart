...
class TaskStatusCubit extends Cubit<JobGenericState<TaskStatusModel>> {
  TaskStatusCubit({required NMRepositoryImpl nmRepositoryImpl})
      : _nmRepositoryImpl = nmRepositoryImpl,
        super(const JobGenericState());

  final NMRepositoryImpl _nmRepositoryImpl;

  void markAsDeleted(TaskStatusModel jobStatus) {
    emit(
      state.copyWith(
  ...
  void updateItemName(TaskStatusModel jobStatus, String name) {
    emit(
      state.copyWith(
        items: state.items?.map((e) {
          if (e == jobStatus) {
  ...
  void updateItemColor(TaskStatusModel jobStatus, String color) {
    emit(
      state.copyWith(
        items: state.items?.map((e) {
          if (e == jobStatus) {
  ...
  void selectItem(TaskStatusModel jobStatus) {
  ...
  Future<void> fetchItems() async {
    ...
    final res = await _nmRepositoryImpl.getTaskStatuses();
